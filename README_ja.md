...
1. **ドロネー三角分割**: シード点を1つずつ追加し、新しい点を外接円内に含む三角形を削除して再三角分割。三角形はハッシュセットで管理し、挿入・削除を O(1) で行うことで高シード数でも高速に動作します。
...
両クリップ段階で AABB（軸平行バウンディングボックス）プレフィルタを使用し、辺の交差判定前に候補ペアの約80%を早期棄却します。5頂点以下の短い境界ウォークでは、コストの高い点内包判定を省略します。
...
各セルに対して：
1. **簡略化**: `corner_radius`（最小 0.01 cm）より短い辺を2パスの線形アルゴリズムで統合 — パス1で前方スキャンにより短辺頂点をマーク除去、パス2で最終頂点と先頭頂点間のラップアラウンド辺を処理。この O(n) 方式により、境界クリッピングで大量の短辺が生じた場合に O(n²) に劣化していた従来の反復ループを置換。
2. **角丸め**: 各凸頂点を、隣接する2辺に接する円弧に置換。フィレット半径は短い方の辺の40%以下にクランプしてオーバーラップを防止。170°超の鈍角は直線のまま維持。
3. **描画**: 直線は `addByTwoPoints`、円弧は `addByThreePoints` を使用。円弧の生成に失敗した場合は自動的に直線にフォールバック。全操作は `isComputeDeferred = True` でバッチ処理し、パフォーマンスを確保。処理中はキャンセルボタン付きのプログレスダイアログを表示し、5セルごとに `adsk.doEvents()` でUIを更新します。
